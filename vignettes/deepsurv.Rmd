...
```
```{r}
ooslkh.ReSurvFit(resurv.fit.xgb)
```
...
```{r}
ooslkh.ReSurvFit(resurv.fit.LTRCtrees)
```
...
```
```{r}
ooslkh.ReSurvFit(resurv.fit.deepsurv)
...
df_output_combined <- bind_rows(
  ...
  arrange(desc(relevant)) %>% 
    ...
    AP_o == min(df_output_combined$AP_o) ~"Initial accident period",
    AP_o == max(df_output_combined$AP_o) ~"Last accident period",
    TRUE   ~"Other accident periods of LTRCTrees" ))  %>% 
  mutate(col= factor(col,levels=c("Other accident periods of LTRCTrees", "Initial accident period" ,"Last accident period") )) %>% 
  mutate(alpha=factor(paste0(Model,AP_o)))
...
scoring_function <-function(resurv.predict){
  ...
  max_dp_i <- max(IndividualData$full.data %>%
                    filter(DP_rev_i > TR_i) %>% select(DP_i))
  ...
  relative_diagnoal = score_diagonal %>% group_by(RP_o) %>%
    summarize(relative_diagnoal = sum(ave_2)/sum(I),
              I=sum(I))

  weighted_relative_diagonal = sum(relative_diagnoal$relative_diagnoal*relative_diagnoal$I)/(sum(relative_diagnoal$I))
...
scoring_function_CL <- function(IndividualData){
  ...
  score_diagonal <- IndividualData$full.data  %>%
    ...
    mutate(I_cum=cumsum(I)) %>%
    mutate(I_cum_lag = lag(I_cum, default = 0)) %>%
    left_join(CL[,c("DP_rev_o","df")], by = c("DP_rev_o")) %>%
