...
vignette: >
  %\VignetteIndexEntry{2024 Reserving Call Paper Program on Technology and the Reserving Actuary}
...
```{r eval=FALSE, include=TRUE}
resurv.predict.xgb <- predict(resurv.fit.xgb)
resurv.predict.nn <- predict(resurv.fit.nn)
```
...
true_output <- resurv.fit.cox$IndividualDataPP$full.data%>%
  mutate(
...
#Total output
score_total<- resurv.fit.predict.Q$long_triangle_format_out$output_granularity[,c("claim_type","AP_o", "DP_o", "I_expected")] %>%
