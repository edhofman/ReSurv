# Package index

## All functions

- [`IndividualDataPP()`](https://edhofman.github.io/ReSurv/reference/IndividualDataPP.md)
  : Individual Data Pre-Processing

- [`ReSurv()`](https://edhofman.github.io/ReSurv/reference/ReSurv.md) :

  Fit `ReSurv` models on the individual data.

- [`ReSurvCV()`](https://edhofman.github.io/ReSurv/reference/ReSurvCV.md)
  : K-fold cross-validation of a ReSurv model

- [`Score_Reserving()`](https://edhofman.github.io/ReSurv/reference/Score_Reserving.md)
  : Score reserving predictions

- [`data_generator()`](https://edhofman.github.io/ReSurv/reference/data_generator.md)
  : Individual data generator

- [`plot(`*`<ReSurvPredict>`*`)`](https://edhofman.github.io/ReSurv/reference/plot.ReSurvPredict.md)
  : Plot of the development factors

- [`predict(`*`<ReSurvFit>`*`)`](https://edhofman.github.io/ReSurv/reference/predict.ReSurvFit.md)
  : Predict IBNR frequency

- [`predictReserve()`](https://edhofman.github.io/ReSurv/reference/predictReserve.md)
  : Predict deterministic reserve table

- [`print(`*`<ReSurvCV>`*`)`](https://edhofman.github.io/ReSurv/reference/print.ReSurvCV.md)
  : Print method for ReSurvCV objects

- [`print(`*`<summaryReSurvPredict>`*`)`](https://edhofman.github.io/ReSurv/reference/print.summaryReSurvPredict.md)
  : Print summary of IBNR predictions

- [`summary(`*`<ReSurvPredict>`*`)`](https://edhofman.github.io/ReSurv/reference/summary.ReSurvPredict.md)
  : Summary of IBNR predictions
